...
if %BuildUWP% equ 0 (
  ...
  if "%TDESKTOP_UPDATE_V2%" equ "1" (
    if %BuildARM% neq 0 (
      call Packer.exe -version %VersionForPacker% -path %BinaryName%.exe -path Updater.exe -path %XrayBinaryName% -target %BuildTarget% -channel %UpdateChannel% -keys-loc "%ResourcesPath%\update" -emit-signing-input signing-input.bin || goto error
    ) else (
      call Packer.exe -version %VersionForPacker% -path %BinaryName%.exe -path Updater.exe -path %XrayBinaryName% -path "modules\%Platform%\d3d\d3dcompiler_47.dll" -target %BuildTarget% -channel %UpdateChannel% -keys-loc "%ResourcesPath%\update" -emit-signing-input signing-input.bin || goto error
    )
